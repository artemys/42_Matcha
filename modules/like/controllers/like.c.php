...
function like($user, $db, $user_id, $scored)
{
	...
	if (!$liked)
	{
		...
		if (isset($row['user_like']))
		{
			...
			{
				$user->notify_entry($db, $scored, $user_id, "conected");
				$user->notify_entry($db, $user_id, $scored, "conected");
			}
		}
	}
}
...
function dislike($user, $db, $user_id, $scored)
{
	...
	if (isset($row['user_like']))
	{
		...
		$array_param = array(':new_like_list'=>$new_like_list, ':scored'=>$scored);
		db_call($sql, $db, $array_param, 0);
		$sql = "DELETE FROM notif WHERE owner_id = :user_id AND guest_id = :scored AND content = 'conected'";
		$array_param = array(':user_id'=>$user_id, ':scored'=>$scored);
		db_call($sql, $db, $array_param, 0);
		$sql = "DELETE FROM notif WHERE owner_id = :scored AND guest_id = :user_id AND content = 'conected'";
		$array_param = array(':user_id'=>$user_id, ':scored'=>$scored);
		db_call($sql, $db, $array_param, 0);
