...
function save_user_loc($db, $user_id, $user_loc)
{
	try
	{
		// $user_test = substr($user_loc, 8);
		$user_loc_tab = explode(',', $user_loc);
		file_put_contents("test23.txt", $user_loc_tab);

		$stmt = $db->conn->prepare("SELECT user_true_long, user_true_lat FROM profils WHERE user_id = :user_id");
		$stmt->execute(array(':user_id'=>$user_id));
		$row = $stmt->fetch(PDO::FETCH_ASSOC);
		if ($row['user_true_long'] != $user_loc_tab[0] || $row['user_true_lat'] != $user_loc_tab[1])
		{
			...
			$stmt->execute(array(':user_id'=>$user_id, ':user_long'=>$user_loc_tab[0], ':user_lat'=>$user_loc_tab[1], ':user_city'=>$user_loc_tab[2], ':user_city_code'=>$user_loc_tab[3], ':user_country'=>$user_loc_tab[4]));
		}
	}
	catch(PDOExeption $e)
	{
...
// 	$_SESSION['user_ip'] = 
// }
// file_put_contents("userloctest1.txt","ip= " .  $_POST['user_ip'] . " ", FILE_APPEND);
// file_put_contents("userloctest1.txt","id= " . $_SESSION['user_id'] . " ", FILE_APPEND);
// file_put_contents("userloctest1.txt","lo= " . $_POST['user_loc'] . " ", FILE_APPEND);
